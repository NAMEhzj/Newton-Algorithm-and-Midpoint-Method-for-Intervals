> module Intervals.RoundedArithmetic where
>
> import Numeric.IEEE
>
> {-
...
>   switching of rounding modes.  The disadvantage is that they do not always yield the tightest possible interval.
...
> add_u x y | isInfinite (x + y)           = x+y
>           | otherwise                    = (x + y) + (((abs (x + y))* phi1) + eta)
>
> add_d :: Double -> Double -> Double
> add_d x y | isInfinite (x + y)           = x + y 
>           | otherwise                    = (x + y) - (((abs (x + y))* phi1) + eta)
>
> sub_u :: Double -> Double -> Double
> sub_u x y | isInfinite (x - y)           = x - y
>           | otherwise                    = (x - y) + (((abs (x - y))* phi1) + eta)
>
> sub_d :: Double -> Double -> Double
> sub_d x y | isInfinite(x - y)            = x - y
>           | otherwise                    = (x - y) - (((abs (x - y))* phi1) + eta)
>
> mul_u :: Double -> Double -> Double
> mul_u x y | isInfinite (x*y)               = x*y
>           | otherwise                    = (x * y) + (((abs (x * y))* phi1) + eta)
>
> mul_d :: Double -> Double -> Double
> mul_d x y | isInfinite  (x*y)              = x*y
>           | otherwise                    = (x * y) - (((abs (x * y))* phi1) + eta)
>
> div_u :: Double -> Double -> Double
> div_u x y | isInfinite (x/y)  = x/y
>           | otherwise       = (x / y)  + (((abs (x / y))* phi1) + eta)
>
> div_d :: Double -> Double -> Double
> div_d x y | isInfinite (x/y)  = x/y
>           | otherwise       = (x / y) - (((abs (x / y))* phi1) + eta)
