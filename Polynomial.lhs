> -- this module gives a possibility to model Polynomials using just a list coefficients 
...
>
>
> -- a polynomial is given by a list of coefficients, stating from the one for x^0 and then x^1 etc up to whatever highest power you want
...
> -- this calculation will be exact since the coefficients will not all of the precision that double provides, 
...
>
>
> -- an evaluation function. It transforms the Polynomial into an Interval 
> -- extension of the function described by it (calculating f(x) = a_0 + x*(a_1 + x*(a_2 + x*(...))) ) 
